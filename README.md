...
This is a client-side wallet library that can generate private keys from a mnemonic, or import private keys from other DIGIWAGE wallets.

It can sign transactions locally, and submit the raw transaction data to a remote digiwage node. The blockchain data is provided by the Insight API (which powers https://explorer.digiwage.org/), rather than the raw digiwaged RPC calls.

This library makes it possible to run DApp without the users having to run a full digiwaged node.

> This library is extracted from the official [DIGIWAGE web wallet](https://github.com/digiwageproject/digiwage-web-wallet).

## Install

```
yarn add digiwagejs-wallet
```

## Running Tests

The digiwagejs-wallet depends on a number of external services. To run the unit tests, you'll need to start the services using [docker-compose](https://docs.docker.com/compose/install/).
...
- Uses satoshi (1e8) as internal units
  - Can represent up to ~90 million DIGIWAGE accurately.
...
```js
import { networks, generateMnemonic } from "digiwagejs-wallet";
...
The currency unit used is `satoshi`. To convert digiwage to satoshi you should multiply the amount you want with `1e8`.

```js
import { networks } from "digiwagejs-wallet";
...
The ABI encoding for the `burnbabyburn()` invokation is `e179b912`. We'll burn 0.05 digiwage, expressed in unit of satoshi.
...
async function main() {
  ...
  const tx = await wallet.contractSend(contractAddress, encodedData, {
    amount: 0.05 * 1e8 // 0.05 digiwage in satoshi
  });
...
Suppose you want to import the public address `qg3HYD8c4bAVLeEzA9t3Ken3Y3Mni1HZSS`. Use `digiwage-cli` to dump the private key from wallet:
...
const amount = 0.15 * 1e8; // 0.15 DIGIWAGE
...
```ts
const tx = await wallet.send(toAddress, amount, {
  // rate is 400 satoshi per byte, or  ~0.004 digiwage/KB, as is typical.
  feeRate: 400
});
```
...
You can decode the raw transaction using `digiwage-cli`:

```
digiwage-cli decoderawtransaction 0100000001a09a921ecb38...
...
```
digiwage-cli decoderawtransaction 0100000001e7b6f2767e8a6...
```
...
```js
import { scrypt } from "digiwagejs-wallet";
...
```js
import { scrypt, params } from "digiwagejs-wallet";
